...
var reverseList = function (head) {
    //该元素必须为null,链表最后一个节点next为null
    //不能为new ListNode() 该元素的值为0 next为null
    let ans = null
    for (let node = head; node !== null; node = node.next) {
        //第一次ans为null，node为第一个节点，创建新节点值为第一个节点值，其next为ans(null)
        //第一次操作完毕后ans, node为第二个节点值，创建新节点 next为反转后的前半部分
